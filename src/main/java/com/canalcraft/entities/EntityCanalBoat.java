package com.canalcraft.entities;
...
public class EntityCanalBoat extends Entity
{
    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.<Integer>createKey(EntityCanalBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.<Integer>createKey(EntityCanalBoat.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.<Float>createKey(EntityCanalBoat.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntityCanalBoat.class, DataSerializers.VARINT);
    ...
    private EntityCanalBoat.Status status;
    private EntityCanalBoat.Status previousStatus;
    private double lastYd;

    public EntityCanalBoat(World worldIn)
    {
        super(worldIn);
    ...
    public EntityCanalBoat(World worldIn, double x, double y, double z)
    {
        this(worldIn);
    ...
    protected void entityInit()
    {
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntityCanalBoat.Type.OAK.ordinal()));
    }
    ...
    {
        if (entityIn instanceof EntityCanalBoat)
        {
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityCanalBoat.Status.UNDER_WATER && this.status != EntityCanalBoat.Status.UNDER_FLOWING_WATER)
        {
     ...
     */
    private EntityCanalBoat.Status getBoatStatus()
    {
        EntityCanalBoat.Status entityboat$status = this.getUnderwaterStatus();
        ...
        {
            return EntityCanalBoat.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityCanalBoat.Status.ON_LAND;
            }
            else
            {
                return EntityCanalBoat.Status.IN_AIR;
            }
        }
    }
     ...
     */
    @Nullable
    private EntityCanalBoat.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double)getLiquidHeight(iblockstate, this.worldObj, blockpos$pooledmutableblockpos))
                        {
                            ...
                            {
                                EntityCanalBoat.Status entityboat$status = EntityCanalBoat.Status.UNDER_FLOWING_WATER;
                                return entityboat$status;
                            }
        ...
        return flag ? EntityCanalBoat.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        if (this.previousStatus == EntityCanalBoat.Status.IN_AIR && this.status != EntityCanalBoat.Status.IN_AIR && this.status != EntityCanalBoat.Status.ON_LAND)
        {
            this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
            this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
            this.motionY = 0.0D;
            this.lastYd = 0.0D;
            this.status = EntityCanalBoat.Status.IN_WATER;
        }
        else
        {
            if (this.status == EntityCanalBoat.Status.IN_WATER)
            {
                d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double)this.height;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityCanalBoat.Status.UNDER_FLOWING_WATER)
            {
                d1 = -7.0E-4D;
                this.momentum = 0.9F;
            }
            else if (this.status == EntityCanalBoat.Status.UNDER_WATER)
            {
                d2 = 0.009999999776482582D;
                this.momentum = 0.45F;
            }
            else if (this.status == EntityCanalBoat.Status.IN_AIR)
            {
                this.momentum = 0.9F;
            }
            else if (this.status == EntityCanalBoat.Status.ON_LAND)
            {
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityCanalBoat.Status.ON_LAND)
                    {
    ...
    public static enum Type
    {
         ...
         */
        public static EntityCanalBoat.Type byId(int id)
        {
        ...
        public static EntityCanalBoat.Type getTypeFromString(String nameIn)
        {
